...
void RGWDedupWorker::do_chunk_dedup(IoCtx &ioctx,
  ...
  for (auto chunk : redundant_chunks) {
    if (check_object_exists(cold_ioctx, chunk.fingerprint) < 0) {
      ...
      // check if the number of references of chunk object exceeds the ref count limit
      chunk_refs_t refs;
      if (get_chunk_refs(cold_ioctx, chunk.fingerprint, refs) < 0) {
        continue;
      }
      chunk_refs_by_object_t* chunk_refs = static_cast<chunk_refs_by_object_t*>(refs.r.get());
      if (chunk_refs->by_object.size() >= MAX_CHUNK_REF_SIZE) {
        ldpp_dout(dpp, 2) << __func__ << " chunk obj(" << chunk.fingerprint <<
          ")'s reference size is reached at MAX_CHUNK_REF_SIZE." <<
          " no more dedup is allowed on this chunk." << dendl;
        continue;
      }
    }
