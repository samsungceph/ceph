...
def test_dedup(event):
    print('RGWDedup Chunk Dedup Test Starts')
    create_file('zero')
    create_file('rand')
    read_thread = threading.Thread(target=read_file, args=('rand', event,))
    ...
    # rand_file keeps remain as hot
    read_thread.start()

    # wait for dedup done
    check_dedup_test_done(event)
...
def test_migration(event):
    print('RGWDedup Object Migration Test Starts')
    create_file('zero')
    create_file('rand')
    read_thread = threading.Thread(target=read_file, args=('zero', event,))
    ...
    # zero_file keeps remain as hot
    read_thread.start()

    # wait for migration done
    check_migration_test_done(event)
...
#    dedup_thread = threading.Thread(target=test_dedup, args=(event,))
#    dedup_thread.start()
#    time.sleep(5)
#    test_state_check('Dedup Test', dedup_thread)
#
#    # 
#    time.sleep(5)
#
#    # 2. Migration test
#    event = threading.Event()
#    migration_thread = threading.Thread(target=test_migration, args=(event,))
#    migration_thread.start()
#    time.sleep(5)
#    test_state_check('Migration Test', migration_thread)
#
#    # 
#    time.sleep(5)
